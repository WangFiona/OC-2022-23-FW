    ...
    DriveSignal update(Pose2d poseEstimate, Pose2d poseVelocity) {
        ...
        if (currentTrajectorySequence != null) {
            if (currentSegmentIndex >= currentTrajectorySequence.size()) {
                RobotLog.ii(TAG_SL, "currentTrajectorySequence size " + currentTrajectorySequence.size());
            ...
            if (currentSegment instanceof TrajectorySegment) {
                ...
                if (!follower.isFollowing()) {
                    RobotLog.ii(TAG_SL, "currentSegmentIndex isFollowing here " + currentSegmentIndex);
                    RobotLog.ii(TAG_SL, "isFollowing time duration " + currentTrajectory.duration());
                    RobotLog.ii(TAG_SL, "isFollowing time elapsed " + follower.elapsedTime());
                    RobotLog.ii(TAG_SL, "isFollowing time time remaining " + (currentTrajectory.duration()-follower.elapsedTime()));

                    double trajEndErrorX = currentTrajectory.end().getX() - poseEstimate.getX();
                    double trajEndErrorY = currentTrajectory.end().getY() - poseEstimate.getY();
                    double trajEndErrorHeading = currentTrajectory.end().getHeading() - poseEstimate.getHeading();
                    RobotLog.ii(TAG_SL, "isFollowing location x " + poseEstimate.getX());
                    RobotLog.ii(TAG_SL, "isFollowing error x " + trajEndErrorX);
                    RobotLog.ii(TAG_SL, "isFollowing error y " + trajEndErrorY);
                    RobotLog.ii(TAG_SL, "isFollowing error heading " + trajEndErrorHeading);
                ...
                if (deltaTime >= currentSegment.getDuration()) {
                    RobotLog.ii(TAG_SL, "currentSegmentIndex time here " + currentSegmentIndex);
                    currentSegmentIndex++;
                    driveSignal = new DriveSignal();
                }
                ...
                if (deltaTime >= currentSegment.getDuration()) {
                    RobotLog.ii(TAG_SL, "currentSegmentIndex time2 here " + currentSegmentIndex);
                    currentSegmentIndex++;
                }
            }
