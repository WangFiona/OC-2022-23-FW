...
@Config
@Autonomous(name="auto5B")
public class auto5B extends LinearOpMode {
        ...
        try {
            ...
            toSquare3 = drive.trajectorySequenceBuilder(start)
                    ...
                    .addSpatialMarker(new Vector2d(1,0), () -> {
                        robot.turret.moveTo((Left? -36 : 60), turretPower);
    ...
    public void AutoBody(WaitLinear lp, boolean Left) throws InterruptedException {
        ...
        robot.hSlides.setPosition(Left? 115f : 92f);
        lp.waitMillis(500);
        ...
        while(System.currentTimeMillis() - startTime < (30000-parkTime) && opModeIsActive() && cLevel >= 1) {
            ...
            robot.hSlides.setPosition(110f);
            robot.turret.moveTo((Left? -52 : 60), turretPower); //-46
            lp.waitMillis(1350);
            /*if(Left)
                lp.waitMillis(1200);
            else
    ...
    public void reset90(WaitLinear lp, boolean Left, int newL, boolean wait) throws InterruptedException {
        if(!wait){
            ...
            robot.hSlides.setPosition((Left ? 85f : 100f));
            //if(wait)
            lp.waitMillis(250);
        }

        robot.turret.moveTo((Left? 87 : -81), turretPower); //87
        if(!wait) {
            robot.hSlides.setPosition((Left ? 85f : 100f));
            //lp.waitMillis(200);
        }
