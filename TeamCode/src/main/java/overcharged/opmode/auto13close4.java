...
public class auto13close4 extends LinearOpMode {
    ...
    float close2 = 7f;
    float far2 = 7f;
    float dumpLength2 = 85f;
    float close = 7f;
    float far = 7f;
    float dumpLength = 79f;
    int Length = 0;
    ...
    Vector2d line, p1, p2, p3, line2, p2p2, scorePos;
    Pose2d s3;
    ...
    public void runOpMode() throws InterruptedException {
        try {
            ...
            p1 = new Vector2d(xVal-3, (Left? 22 : 26));
            p2 = new Vector2d(xVal-5, (Left? -3 : 0)); //xVal-3
            p2p2 = new Vector2d(xVal-8, (Left? 0 : 3));
            p3 = new Vector2d(xVal-(Left? 2 : 4), -25);
            ...
            toSquare3 = drive.trajectorySequenceBuilder(start)
                    ...
                    .addSpatialMarker(new Vector2d((Left? 16 : 17),0), () -> {
                        robot.alignerOut();
                        //robot.hSlides.setPosition(Left? 132f : 142f);
                    })
                    .addSpatialMarker(new Vector2d((Left? 20 : 17), 0), () -> {
                        robot.turret.moveTo((Left? -165 : 158), turretPower);//, Left? false : true);//-154 : 151), turretPower);
                    })
                    .build();
            ...
            correct = drive.trajectorySequenceBuilder(new Pose2d(line2.getX(), line2.getY()+(Left? -1 : 1), Left? Math.toRadians(90) : Math.toRadians(-90)))
                    .setVelConstraint(SampleMecanumDrive.getVelocityConstraint(90, Math.PI * 2, DriveConstants.TRACK_WIDTH))
                    .setAccelConstraint(SampleMecanumDrive.getAccelerationConstraint(70))
                    .lineToLinearHeading(new Pose2d(line, Left? Math.toRadians(98) : Math.toRadians(-90)))//line)
                    .build();
            ...
            score = drive.trajectorySequenceBuilder(correct.end())
                    .lineTo(scorePos)
                    .build();

            to1 = drive.trajectorySequenceBuilder(correct.end())
                    .strafeTo(p1)
                    .build();

            toR1 = drive.trajectorySequenceBuilder(correct.end())
                    .strafeTo(p1)
                    .addSpatialMarker(new Vector2d(xVal-2,4), () -> {
                        lowerSlidesThread(lp, 1);})
                    .build();

            to2 = drive.trajectorySequenceBuilder(correct.end())
                    .strafeTo(p2)
            ...
            to3 = drive.trajectorySequenceBuilder(correct.end())
                    .strafeTo(p3)
                    //.addSpatialMarker(new Vector2d(-2,-4), () -> {
                    //lowerSlidesThread(lp, 1);})
                    .build();

            toR3 = drive.trajectorySequenceBuilder(correct.end())
                    .strafeTo(p3)
                    //.addSpatialMarker(new Vector2d(xVal-3,2), () -> {
                    //lowerSlidesThread(lp, 1);})
                    .build();
    ...
    public void AutoBody(WaitLinear lp, boolean Left) throws InterruptedException {
        ...
        robot.clawGrab();
        robot.hSlides.setPosition(dumpLength);//79f : 100f);//(Left? 129f : 142f);//132
        //robot.hSlides.setPosition(100f);
        drive.followTrajectorySequence(toSquare3);

        //robot.hSlides.setPosition(Left? 137f : 142f);//Left? 115f : 92f);
        //lp.waitMillis(200);
        //robot.turret.moveTo((Left? 50 : -47), turretPower); //40,-44
        //lp.waitMillis(300);
        ...
        while (System.currentTimeMillis() - startTime < (30000 - parkTime) && opModeIsActive() && cLevel >= 1) {
            ...
            robot.vSlides.moveTo(1980);//moveTo(1980);//moveTo4();
            lp.waitMillis(200);
            ...
            robot.alignerOut();
            robot.hSlides.setPosition(dumpLength2);//137f : 137f);//(Left? 188f : 179f);//183f
            //lp.waitMillis(150);
            robot.turret.moveTo((Left ? -135 : 135), 0.5f);
        ...
        lp.waitMillis(500);
        /*if ((Left && signalColors == SignalColors.Green) || (!Left && signalColors == SignalColors.Red)) {
            //lp.waitMillis(200);
            lowerSlidesThread(lp, 1);
            lp.waitMillis(200);
        ...
        robot.turret.moveEncoderTo(0, 0.9f);
    }
    ...
    public void reset90(WaitLinear lp, boolean Left, int newL, boolean wait) throws InterruptedException {
        ...
        if(!wait){
            ...
            robot.hSlides.setPosition(Left? 129f : 129f);//140f);
            lp.waitMillis(300);
        }
        ...
        if(!wait) {
            robot.hSlides.setPosition(Left? 131f : 140f);//136f//(Left? 143f : 145f);//(Left ? 85f : 100f));
        }

        lp.waitMillis(200);//84.5, 81.4
        ...
        if((Left && robot.turret.getCurrentAngle() >= 5) || (!Left && robot.turret.getCurrentAngle() <= 5)){//robot.turret.getCurrentAngle()+360 >= 370){
            ...
            resetAngle = robot.turret.getCurrentAngle() - (Left ? 2 : 3);
        ...
        robot.turret.moveTo((Left? 2 : 1), turretPower); //87
        lp.waitMillis(150);
        if(!wait) {
