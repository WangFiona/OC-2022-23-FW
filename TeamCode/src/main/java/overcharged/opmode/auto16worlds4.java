...
public class auto16worlds4 extends LinearOpMode {
    ...
    double resetAngle = -6;
    int offset = 60;
    float dumpLengthL = 161f;//79f;
    float dumpLength2L = 172f;
    float dumpLengthR = 161f;//89f;
    float dumpLength2R = 172f;
    float close = 12f;
    float far = 15f;
    int Length = 0;
    ...
    public void runOpMode() throws InterruptedException {
        try {
            ...
            double yVal = (Left? 1: -12); //-6
            double yVal2 = (Left? 1: -6); //-6
            ...
            toSquare3 = drive.trajectorySequenceBuilder(start)
                    .setVelConstraint(SampleMecanumDrive.getVelocityConstraint(57, Math.PI * 2, DriveConstants.TRACK_WIDTH))
                    .lineToLinearHeading(s3)
                    .addSpatialMarker(new Vector2d((Left? 1 : 12),0), () -> {
                        robot.vSlides.moveTo(1460);
                    })
                    .addSpatialMarker(new Vector2d((Left? 2 : 13), 0), () -> {
                    ...
                    .addSpatialMarker(new Vector2d((Left? 43 : 43),0), () -> {
            ...
            toLine = drive.trajectorySequenceBuilder(toSquare3.end())
                    ...
                    .lineToLinearHeading(new Pose2d(line, Left? Math.toRadians(88) : Math.toRadians(-88)))
                    //.strafeTo(line)
                    .build();
            ...
            correct2 = drive.trajectorySequenceBuilder(new Pose2d(line.getX(), line.getY()+(Left? -1 : 1), Left? Math.toRadians(90) : Math.toRadians(-90)))
                    .lineToLinearHeading(new Pose2d(line2, Left? Math.toRadians(88) : Math.toRadians(-88)))
                    .build();
    ...
    public void AutoBody(WaitLinear lp, boolean Left) throws InterruptedException {
        ...
        while(System.currentTimeMillis() - startTime < (30000-parkTime) && opModeIsActive() && cLevel >= 1 && notGrab < 2 && numGrabbed<5) {
            ...
            robot.vSlides.moveTo(1000);//moveTo4();
            lp.waitMillis(200);
            ...
            robot.turret.moveTo((Left? -172 : 172), turretPower); //-46
    ...
    public void reset90(WaitLinear lp, boolean Left, int newL, boolean wait) throws InterruptedException {
        robot.turret.turret.setTargetPositionPIDFCoefficients(7,0,0,0);
        robot.hSlides.setPosition(Left? 20f : 20f);//140f);
        robot.aligner.setLeft();
        robot.alignerInit();
        ...
        if(wait) {
            ...
            robot.hSlides.setPosition(Left? 142f : 142f);//136f//(Left? 143f : 145f);//(Left ? 85f : 100f));
        } else {
            robot.hSlides.setPosition(Left? 162f : 162f);//136f//(Left? 143f : 145f);//(Left ? 85f : 100f));
            drive.followTrajectorySequence(toLine);
        }
