...
public class auto3 extends LinearOpMode {
    ...
    TrajectorySequence toSquare3, toLine, toLine2, to1, to2, to3, toScore, correct, correct2, to2p2, score;
        ...
        try {
            ...
            toSquare3 = drive.trajectorySequenceBuilder(start)
                    ...
                    .addSpatialMarker(new Vector2d(1,0), () -> {
                        ...
                        robot.hSlides.setPosition(Left? 128f : 115f);})
                    .build();
            ...
            correct = drive.trajectorySequenceBuilder(new Pose2d(line.getX(), line.getY()+(Left? -1 : 1), 0))
                    .lineTo(line)
                    .addSpatialMarker(new Vector2d(xVal, (Left? -4 : 4)), () -> {
                        robot.turret.moveTo((Left? 86 : -79.5), turretPower); //87
                        robot.hSlides.setPosition((Left ? 85f : 105f));
                    })
                    .build();
            ...
            to1 = drive.trajectorySequenceBuilder(new Pose2d(scorePos.getX(), scorePos.getY()+(Left? -1 : 1), 0))
                    .strafeTo(p1)
                    .build();

            to2 = drive.trajectorySequenceBuilder(new Pose2d(scorePos.getX(), scorePos.getY()+(Left? -1 : 1), 0))
                    .strafeTo(p2)
            ...
            to3 = drive.trajectorySequenceBuilder(new Pose2d(scorePos.getX(), scorePos.getY()+(Left? -1 : 1), 0))
                    .strafeTo(p3)
    ...
    public void AutoBody(WaitLinear lp, boolean Left) throws InterruptedException {
        ...
        robot.hSlides.setPosition(Left? 115f : 105f);
        lp.waitMillis(500);
        ...
        while(System.currentTimeMillis() - startTime < (30000-parkTime) && opModeIsActive() && cLevel >= 1) {
            ...
            robot.hSlides.setPosition(146f);
            robot.turret.moveTo((Left? 0 : 0), turretPower); //-46
            drive.followTrajectorySequence(score);
            //lp.waitMillis(1100);

            if(robot.sensorF.getDistance(DistanceUnit.CM) < 10 && grabbed) {
                robot.hSlides.setPosition(130f);
    ...
    public void reset90(WaitLinear lp, boolean Left, int newL, boolean wait) throws InterruptedException {
        if(!wait){
            ...
            robot.hSlides.setPosition((Left ? 149f : 105f));
            //if(wait)
            //lp.waitMillis(700);
        }
