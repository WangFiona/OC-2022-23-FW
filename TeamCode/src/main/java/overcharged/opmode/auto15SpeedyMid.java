...
public class auto15SpeedyMid extends LinearOpMode {
    ...
    float dumpLengthR = 79f;
    float dumpLength2R = 127f;
    float close = 12f;
    float far = 15f;
    int Length = 0;
    int numGrabbed = 0;
        ...
        try {
            ...
            toSquare3 = drive.trajectorySequenceBuilder(start)
                    ...
                    .addSpatialMarker(new Vector2d((Left? 20 : 20), 0), () -> {
                        robot.turret.moveTo((Left? -163 : 160), turretPower);//, Left? false : true);//-154 : 151), turretPower);
                    })
                    .addSpatialMarker(new Vector2d((Left? 30 : 30),0), () -> {
                        robot.alignerOut();
                    })
                    .addSpatialMarker(new Vector2d((Left? 48 : 48),0), () -> {
            ...
            correct = drive.trajectorySequenceBuilder(new Pose2d(line.getX(), line.getY()+(Left? -1 : 1), Left? Math.toRadians(90) : Math.toRadians(-90)))
                    .lineToLinearHeading(new Pose2d(line, Left? Math.toRadians(93) : Math.toRadians(-94)))
                    .build();
    ...
    public void AutoBody(WaitLinear lp, boolean Left) throws InterruptedException {
        ...
        while(System.currentTimeMillis() - startTime < (30000-parkTime) && opModeIsActive() && cLevel >= 1 && notGrab < 2 && numGrabbed<5) {
            ...
            robot.turret.turret.setTargetPositionPIDFCoefficients(2, 0, 0, 0);//1.4
            ...
            robot.turret.moveTo((Left ? -170f : 175f), turretPower);//, Left? false : true);//-180 : 180), turretPower); //-46
            lp.waitMillis(700);
        ...
        robot.turret.turret.setTargetPositionPIDFCoefficients(8.5, 0, 0, 0);
    ...
    public void reset90(WaitLinear lp, boolean Left, int newL, boolean wait) throws InterruptedException {
        ...
        //if(!wait) {
        lp.waitMillis(300);
        robot.hSlides.setPosition(Left? 130f : 150f);//136f//(Left? 143f : 145f);//(Left ? 85f : 100f));
        //}

        lp.waitMillis(300);//84.5, 81.4
        ...
        telemetry.update();
        while(!wait && robot.sensorF.getDistance(DistanceUnit.CM) > 9 && ((Left && robot.turret.getCurrentAngle() < 5) || (!Left && robot.turret.getCurrentAngle() > 5))){//robot.turret.getCurrentAngle()+360 < 370){
            robot.turret.turret.setMode(DcMotor.RunMode.RUN_USING_ENCODER);
            robot.turret.setPower(Left? 0.3f : -0.3f);
        ...
        if((Left && robot.turret.getCurrentAngle() >= 5) || (!Left && robot.turret.getCurrentAngle() <= 5)){//robot.turret.getCurrentAngle()+360 >= 370){
            resetAngle = 0;
            robot.turret.moveTo(resetAngle, turretPower);
        } else if(!wait) {
