...
@Disabled
@Autonomous(name="auto12backandforth")
public class auto12backandforth extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        try {
            ...
            double yVal = (Left? 14: -9); //-6
            double yVal2 = (Left? -13: 13); //8.5
            ...
            p3 = new Vector2d(xVal-(Left? 1 : 4), -26);
            ...
            toSquare3 = drive.trajectorySequenceBuilder(start)
                    ...
                    .addSpatialMarker(new Vector2d((Left? 3 : 3),0), () -> {
                        robot.vSlides.moveTo(1980);
                        robot.alignerOut();
                        //robot.hSlides.setPosition(Left? 132f : 142f);
                    })
                    .addSpatialMarker(new Vector2d((Left? 30 : 17), 0), () -> {
                        robot.turret.moveTo((Left? 145 : -145), turretPower);
                    })
                    .build();
            ...
            correct = drive.trajectorySequenceBuilder(new Pose2d(line2.getX(), line2.getY()+(Left? -1 : 1), Left? Math.toRadians(90) : Math.toRadians(-90)))
                    .lineToLinearHeading(new Pose2d(line.getX(), (Left? 15: -13), Left? Math.toRadians(90) : Math.toRadians(-90)))//line)
                    .build();
    ...
    public void AutoBody(WaitLinear lp, boolean Left) throws InterruptedException {
        ...
        robot.clawGrab();
        robot.hSlides.setPosition(Left? 112f : 104f);//112f//(Left? 129f : 142f);//132
        ...
        while(System.currentTimeMillis() - startTime < (30000-parkTime) && opModeIsActive() && cLevel >= 1 && notGrab < 2 && numGrabbed<5) {
            ...
            robot.turret.turret.setTargetPositionPIDFCoefficients(0.8,0,0,0);//1.4
            ...
            robot.alignerOut();
            robot.hSlides.setPosition(Left? 92f : 92f);//157f : 156f);
            robot.turret.moveTo((Left? 99 : -99), turretPower);
            lp.waitMillis(150);
            drive.followTrajectorySequence(correct2);
            lp.waitMillis(150);
            //robot.turret.moveTo((Left? 90 : -90), turretPower); //-46
    ...
    public void reset90(WaitLinear lp, boolean Left, int newL, boolean wait) throws InterruptedException {
        ...
        if(!wait){
            ...
            robot.hSlides.setPosition(Left? 70f : 70f);//140f);
            lp.waitMillis(75);
        }
        ...
        if(!wait) {
            robot.hSlides.setPosition(Left? 116f : 130f);//136f//(Left? 143f : 145f);//(Left ? 85f : 100f));
        }

        robot.vSlides.moveTo(cone1+(interval*newL));
        if(wait) {
            lp.waitMillis(200);
            robot.hSlides.setPosition(Left? 105f : 105f);//140f);
            drive.followTrajectorySequence(correct);
            //lp.waitMillis(200);
        } else
            drive.followTrajectorySequence(toLine);
        ...
        telemetry.update();
        while(!wait && robot.sensorF.getDistance(DistanceUnit.CM) > 8 && ((Left && robot.turret.getCurrentAngle() > -resetAngle) || (!Left && robot.turret.getCurrentAngle() < -resetAngle))){
        ...
        if((Left && robot.turret.getCurrentAngle() < -resetAngle) || (!Left && robot.turret.getCurrentAngle() < -resetAngle)){
            resetAngle = 0;
            robot.turret.moveTo((Left? resetAngle : resetAngle), turretPower); //84.5, 81.4
        } else if(!wait) {
